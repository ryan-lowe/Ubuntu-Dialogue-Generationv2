...
class CreateDataset:
  ...
  def checkValidity(self, c2, percentage, convo): 
    ...
    for user,value in uniqueuser.iteritems():
      if value < percentage*len(userlist) and len(userlist) > 5:
        return False
        self.writeFiles('/home/ryanlowe/src/deletedfiles.csv', [convo])
    return True
  ...
  def createDicts(self, testpct, trainfiles = None, valfiles = None, testfiles = None):
    ...
    if trainfiles == None:
      ...
      for i in xrange(int(len(self.filelist)*(1-2*testpct))):
        ...
        self.writeFiles('/home/ryanlowe/src/trainfiles.csv', [self.filelist[i]])
      for i in xrange(int(len(self.filelist)*(1-2*testpct)), int(len(self.filelist)*(1-testpct))):
        self.traindic[self.filelist[i][0] + self.filelist[i][1]] = self.filelist[i][1]
        self.writeFiles('/home/ryanlowe/src/valfiles.csv', [self.filelist[i]])
      for i in xrange(int(len(self.filelist)*(1-testpct)), len(self.filelist)):
        ...
        self.writeFiles('/home/ryanlowe/src/testfiles.csv', [self.filelist[i]])
    else:
    ...
    for row in c2:
      ...
      if len(row) > 3:
        ...
        if len(row[1]) == 0:
          namedict['error'] = 0
    if len(namedict) > 2:
      self.writeFiles('/home/ryanlowe/src/badfiles.csv', [[filein]])  
  ...
  def appendTestData(self, utterlist, check_dict, max_context_size, convo, testpct, num_options_test, val=None):
    ...
    for i in xrange(0, int((len(utterlist))/contextsize)):
      ...
      #self.writeFiles('/home/ryanlowe/src/testfiles.csv', [[convo,contextsize-1]])   
  ...
  def sortFiles(self, max_context_size=20, num_options_train=2, num_options_test=2, testpct=0.1, filesperprint=100, elimpct=0.2, badfiles=False):            
    ...
    self.writeFiles('/home/ryanlowe/src/trainset.csv', [], overwrite = True)
    self.writeFiles('/home/ryanlowe/src/valset.csv', [], overwrite = True)        
    self.writeFiles('/home/ryanlowe/src/testset.csv', [], overwrite = True)   
    self.writeFiles('/home/ryanlowe/src/turnlist.csv', [], overwrite = True)       
    self.writeFiles('/home/ryanlowe/src/timelist.csv', [], overwrite = True)       
    self.writeFiles('/home/ryanlowe/src/badfiles.csv', [], overwrite = True)    
        ...
        for convo in files:
          ...
          with open(filein, 'r') as c1:
            ...
            if self.checkValidity(c2, elimpct, convo):
              ...
              if len(utterlist) < 3:
                self.writeFiles('/home/ryanlowe/src/badfiles.csv',[[convo]])
              else:
              ...
              if i % 10 == 0:#k % filesperprint == 0 or k == len(files):
                if self.traindata != []:
                  self.writeFiles('/home/ryanlowe/src/trainset.csv', self.traindata, listbool=True)
                if self.valdata != []:
                  self.writeFiles('/home/ryanlowe/src/valset.csv', self.valdata, listbool = True)
                if self.testdata != []:
                  self.writeFiles('/home/ryanlowe/src/testset.csv', self.testdata, listbool=True)
                self.writeFiles('/home/ryanlowe/src/turndata.csv', [(self.timelist, self.turnlist, self.dictlist)])  
                self.traindata = []
                self.valdata = []
                self.testdata = []
                self.timelist = []
  ...
  def sortFilesParallel(self, dialoguefile, seg_index, max_context_size=20, num_options_train=2, num_options_test=2, testpct=0.1, filesperprint=100, elimpct=0.2, badfiles=False):            
    #firstline = [['Context','Response','Correct']]
    seg_index = str(seg_index)
    self.writeFiles('/home/ryanlowe/src/trainset_'+seg_index+'.csv', [], overwrite = True)
    self.writeFiles('/home/ryanlowe/src/valset_'+seg_index+'.csv', [], overwrite = True)        
    self.writeFiles('/home/ryanlowe/src/testset_'+seg_index+'.csv', [], overwrite = True)   
    self.writeFiles('/home/ryanlowe/src/turndata_'+seg_index+'.csv', [], overwrite = True)       
    self.writeFiles('/home/ryanlowe/src/badfiles_'+seg_index+'.csv', [], overwrite = True)    
    k=0
    i=0
        ...
        if int(folder) > 2:
          ...
          with open(filein, 'r') as c1:
            ...
            if self.checkValidity(c2, elimpct, convo):
              utterlist = self.concatUtter(utterlist, userlist)
              if len(utterlist) < 3:
                self.writeFiles('/home/ryanlowe/src/badfiles_'+seg_index+'.csv',[[convo]])
              else:
                if utterlist[0] != utterlist[1]: #checks for ubotu utterance, and for 'good' dialogue           
                  self.turnlist.append(len(utterlist))
                  self.makeTimeList(c2)
              ...
              if k % filesperprint == 0 or folder + convo == lastfold:
                if self.traindata != []:
                  self.writeFiles('/home/ryanlowe/src/trainset_'+seg_index+'.csv', self.traindata, listbool=True)
                if self.valdata != []:
                  self.writeFiles('/home/ryanlowe/src/valset_'+seg_index+'.csv', self.valdata, listbool = True)
                if self.testdata != []:
                  self.writeFiles('/home/ryanlowe/src/testset_'+seg_index+'.csv', self.testdata, listbool=True)
                for i in range(len(self.timelist)):
                  self.writeFiles('/home/ryanlowe/src/turndata_'+seg_index+'.csv', [(self.timelist[i], self.turnlist[i], self.dictlist[i])])             
                self.traindata = []
                self.valdata = []
                self.testdata = []
                self.timelist = []
              ...
              if folder + convo == lastfold:
                ...
                if self.valdata != []:
                  self.writeFiles('/home/ryanlowe/src/valset_'+seg_index+'.csv', self.valdata, listbool = True)
                if self.testdata != []:
                  self.writeFiles('/home/ryanlowe/src/trainset_'+seg_index+'.csv', self.testdata, listbool=True)
                for i in range(len(self.timelist)):
                  self.writeFiles('/home/ryanlowe/src/turndata_'+seg_index+'.csv', [(self.timelist[i], self.turnlist[i], self.dictlist[i])])             
                self.traindata = []
                self.valdata = []
